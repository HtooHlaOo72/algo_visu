...
def sort():
    ...
    if (algorithm.get() == "Merge Sort"):
        sort_thread = Thread(target=sort_fn, args=(data, 0, len(
            data)-1, speed_num, drawGraph, increase_step, control))
    else:
        sort_thread = Thread(target=sort_fn, args=(
            data, speed_num, drawGraph, increase_step, control))
    sort_thread.start()
